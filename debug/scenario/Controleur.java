...
import debug.scenario.ihm.FrameCartes;
import debug.scenario.ihm.FrameGame;
import debug.scenario.metier.*;
...
public class Controleur implements WindowStateListener
{
	...
	public Controleur(int scenario)
	{
		...
		/*
			Scénario possible :
			1. Carte Noir pioché dès le début
			2. Carte Noir pioché à la fin
			3. Cycle
			4. Croisé
			5. Dégeugage
		 */
		this.metier1.setScenario(scenario);

		if (scenario == 5)
		{
			this.montrerCarte = true;
		}
	}
	...
	{
		this.ihmPioche.piocherForce(qtt);
	}
	...
	public String getImage(int indice)
	{
		...
		if (c != null && (!c.estCache() || this.montrerCarte))
